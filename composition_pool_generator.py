...
def remove_duplicates(df_new, df_existing, element_cols, decimals=6):
    # Remove rows from df_new that match any row in df_existing
...
def filter_invalid_compositions(df, min_elements, max_elements):
    # Make sure that the number of non zero elements are within limits
    ...
    # Remove compositions that do not sum to 100
...
def generate_doe(concentration_values, num_processes, elements, n_elements, min_elements, max_elements):
    # Generate the grid design of experiments
...
def main():
    ...
    df_combined.to_hdf('compositions_dataset_b.h5', key='df')
