"""Postprocessing helpers for CALPHAD outputs.

Functions to compute phase ordering, normalize phase fractions and
derive A2/B2 aggregated fractions and compositions used by downstream
...
def compute_A2_B2_and_A2_composition(df, max_b2_index=3, tol=1e-12):
    ...
    B2_glob = np.sum(np.where(phase_orders[:, :, None] == 1, phase_fracs[:, :, None] * phase_comps, 0.0), axis=0)
    
    # Normalize compositions within A2 and B2
    X_A2 = np.full_like(A2_glob, 0)
    X_B2 = np.full_like(B2_glob, 0)
    ...
    rows_b2 = np.where(B2_frac > tol)[0]
    if rows_b2.size:
        XB2_rows = B2_glob[rows_b2, :] / B2_frac[rows_b2, None]
        XB2_rows /= np.where(XB2_rows.sum(axis=1, keepdims=True) > tol, XB2_rows.sum(axis=1, keepdims=True), 1.0)
        X_B2[rows_b2, :] = XB2_rows

    # Output per-element compositions within A2
    for j, el in enumerate(elements):
        out[f"{el}_in_A2"] = X_A2[:, j]
    for j, el in enumerate(elements):
        out[f"{el}_in_B2"] = X_B2[:, j]
